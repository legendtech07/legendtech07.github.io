...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Escape any value before it is placed into innerHTML, so data stored by a
    // user (username, email, etc.) can never inject markup/scripts into the admin panel.
    ...
    // isAdmin: true. This must be enforced server-side too — set Firestore
    // Security Rules so a normal user can never write isAdmin on their own
    // document; only set it by hand (Firebase console) or from trusted
    // backend code. There is intentionally no client-side password list here:
    // anything shipped in this file is visible to anyone who opens dev tools.
