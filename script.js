...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // LP Packages in Naira (Christmas Special Pricing)
    const lpPackages = [
    ...
    // UPDATED: Promo Codes - Can only be used once per user (Christmas Special)
    const promoCodes = {
        'UPDATE': 500,
        'WELCOME100': 100,
        'LEGEND500': 500,
        'MERRIAS': 1000  // NEW Christmas promo code
    };
    
    // Global chat messages
    const globalChatMessages = [
        { username: "Admin", message: "🎄 Welcome to Legend Tech community chat! Merry Christmas!", time: "10:00 AM" },
        ...
        { username: "Admin", message: "🎅 Christmas Special: Use promo code 'MERRIAS' for 1000 LP!"}
    ];
        ...
        showNewNotificationPopup(notification) {
            ...
            popup.innerHTML = `
                <div style="display: flex; align-items: flex-start; gap: 10px;">
                    ...
                    <div style="flex: 1;">
                        <div style="font-weight: bold; margin-bottom: 5px; font-size: 14px; color: var(--christmas-gold);">${notification.title}</div>
                    ...
                    ">
                        <div class="announcement-badge priority-${announcement.priority}" style="
                            background: ${announcement.priority === 'high' ? 'var(--danger)' : 
                                       announcement.priority === 'medium' ? 'var(--warning)' : 'var(--christmas-gold)'};
                        ...
                        <h2 class="announcement-header" style="color: var(--christmas-gold); margin-bottom: 1rem; font-size: 1.5rem;">
                        ...
                        <div class="announcement-buttons" style="display: flex; gap: 1rem; justify-content: space-between; align-items: center;">
                            ...
                            <button class="btn btn-next" style="background: var(--christmas-gold); border: none; color: black; padding: 0.8rem 1.5rem; border-radius: 8px; cursor: pointer; font-weight: bold;">
            ...
            announcementsList.innerHTML = latestAnnouncements.map(announcement => `
                <div class="announcement-item">
                    <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 0.5rem;">
                        <div>
                            <div style="font-weight: bold; color: var(--christmas-gold); margin-bottom: 0.3rem;">
                                ${this.getTypeIcon(announcement.type)} ${announcement.title}
                            </div>
    ...
    function addLogoutButton() {
        ...
        if (!userInfo.querySelector('.logout-btn')) {
            ...
            logoutBtn.style.background = 'rgba(214, 40, 40, 0.2)';
            logoutBtn.style.border = '1px solid rgba(214, 40, 40, 0.3)';
            logoutBtn.style.color = 'var(--christmas-red)';
        ...
        try {
            ...
            if (doc.exists) {
                ...
                if (!currentUser.likedCourses) {
                    currentUser.likedCourses = [];
                }
                if (!currentUser.dislikedCourses) {
                    currentUser.dislikedCourses = [];
                }
                if (!currentUser.claimedAirdrops) {
                    currentUser.claimedAirdrops = [];
                }
            ...
            if (isUnlocked) {
                courseContent = `
                    ...
                    <div class="course-interactions">
                        <button class="interaction-btn like-btn ${hasLiked ? 'liked' : ''}" data-course-id="${course.id}">
                            <i class="fas fa-thumbs-up"></i>
                            <span class="like-count">${course.likes || 0}</span>
                        </button>
                        <button class="interaction-btn dislike-btn ${hasDisliked ? 'disliked' : ''}" data-course-id="${course.id}">
                    ...
                    <div class="course-meta">
                        ...
                        <a href="${course.url}" target="_blank" class="btn"><i class="fas fa-play"></i> Start Learning</a>
                    </div>
                `;
            } else {
                courseContent = `
                    ...
                    ` : ''}
                    <div class="course-meta">
                        <div class="course-price">${course.price || 100} LP</div>
                        <button class="btn" data-course-id="${course.id}"><i class="fas fa-lock-open"></i> Unlock</button>
                    </div>
                `;
            }
    ...
    closeAirdropModal.addEventListener('click', closeModals);
    closeAirdropBtn.addEventListener('click', closeModals);
        ...
        try {
            ...
            if (type === 'like') {
                ...
                if (currentUser.likedCourses.includes(courseId)) {
                    ...
                    // Remove TP from tutor if this was their course
                    if (courseData.tutorId) {
                    ...
                    if (currentUser.dislikedCourses.includes(courseId)) {
                        ...
                        // Adjust tutor points for removed dislike
                        if (courseData.tutorId) {
                            tutorUpdateData.tutorPoints = firebase.firestore.FieldValue.increment(1);
                    ...
                    // Add TP to tutor if this is their course
                    if (courseData.tutorId) {
                        tutorUpdateData.tutorPoints = firebase.firestore.FieldValue.increment(1);
                        tutorUpdateData.totalLikes = firebase.firestore.FieldValue.increment(1);
                    }
                ...
                if (currentUser.dislikedCourses.includes(courseId)) {
                    ...
                    if (currentUser.likedCourses.includes(courseId)) {
                        ...
                        if (courseData.tutorId) {
                            tutorUpdateData.tutorPoints = firebase.firestore.FieldValue.increment(-2);
                            tutorUpdateData.totalLikes = firebase.firestore.FieldValue.increment(-1);
                        }
                    }
    ...
    // Add chat message
    function addChatMessage(username, message, time = null, isUserMessage = false) {
    ...
    function showSuccess(message, isSuccess = true) {
        ...
        successMessage.style.color = isSuccess ? 'var(--christmas-green)' : 'var(--danger)';
