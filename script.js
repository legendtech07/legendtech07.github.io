...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // UPDATED: Initialize chat - now uses global chat
    ...
    async function loadGlobalChat() {
        try {
            ...
            snapshot.forEach(doc => {
                const msg = doc.data();
                addChatMessage(msg.username, msg.message, msg.time, false);
            });
        } catch (error) {
                ...
                snapshot.docChanges().forEach((change) => {
                    if (change.type === 'added') {
                        ...
                        if (!document.querySelector(`[data-message-id="${change.doc.id}"]`)) {
                            addChatMessage(msg.username, msg.message, msg.time, false, change.doc.id);
                        }
                    }
                });
            });
    ...
    function addChatMessage(username, message, time = null, isUserMessage = false, messageId = null) {
    ...
    // UPDATED: Send message to global chat
