...
document.addEventListener('DOMContentLoaded', function() {
        ...
        showNewNotificationPopup(notification) {
            ...
            popup.innerHTML = `
                <div style="display: flex; align-items: flex-start; gap: 10px;">
                    ...
                    <div style="flex: 1;">
                        <div style="font-weight: bold; margin-bottom: 5px; font-size: 14px; color: var(--accent);">${notification.title}</div>
                        <div style="font-size: 12px; opacity: 0.9; color: white;">${notification.message}</div>
                    </div>
            ...
            this.notifications.forEach(notification => {
                ...
                notificationItem.innerHTML = `
                    ...
                    <div class="notification-content">
                        <p style="margin: 0 0 5px 0; font-weight: bold; font-size: 13px;">${notification.title}</p>
                        <p style="margin: 0 0 5px 0; font-size: 12px;">${notification.message}</p>
                        <small>${timeAgo}</small>
                    </div>
                `;
                ...
                modal.innerHTML = `
                    ...
                    ">
                        ...
                        <div class="announcement-buttons" style="display: flex; gap: 1rem; justify-content: space-between; align-items: center;">
                            ...
                            <button class="btn btn-next" style="background: #00d2ff; border: none; color: white; padding: 0.8rem 1.5rem; border-radius: 8px; cursor: pointer; font-weight: bold;">
    ...
    // NEW: Create reply container
    function createReplyContainer() {
        const replyContainer = document.createElement('div');
        replyContainer.id = 'reply-container';
        replyContainer.style.display = 'none';
        replyContainer.style.padding = '0.5rem';
        replyContainer.style.background = 'rgba(74, 0, 224, 0.1)';
        replyContainer.style.borderLeft = '3px solid var(--primary)';
        replyContainer.style.marginBottom = '0.5rem';
        replyContainer.style.borderRadius = '5px';
        replyContainer.style.fontSize = '0.8rem';
        
        const replyContent = document.createElement('div');
        replyContent.id = 'reply-content';
        replyContent.style.display = 'flex';
        replyContent.style.justifyContent = 'space-between';
        replyContent.style.alignItems = 'center';
        
        const replyText = document.createElement('span');
        replyText.id = 'reply-text';
        
        const cancelReplyBtn = document.createElement('button');
        cancelReplyBtn.innerHTML = '✕';
        cancelReplyBtn.style.background = 'none';
        cancelReplyBtn.style.border = 'none';
        cancelReplyBtn.style.color = 'var(--danger)';
        ...
        replyContent.appendChild(cancelReplyBtn);
        replyContainer.appendChild(replyContent);
        
        // Insert reply container before chat input
        const chatInputContainer = chatInput.parentElement;
        chatInputContainer.parentElement.insertBefore(replyContainer, chatInputContainer);
        
        return replyContainer;
    }
    
    // Initialize reply container
    ...
    async function loadCoursesFromFirestore() {
        try {
            ...
            const allCourses = [...courses, ...firestoreCourses];
    ...
    // UPDATED: Initialize chat - now with reply and mention features
    ...
    // NEW: Load global chat from Firestore
    async function loadGlobalChat() {
        try {
            ...
            snapshot.forEach(doc => {
                const msg = doc.data();
                addChatMessage(msg.username, msg.message, msg.time, false, doc.id, msg.replyTo);
            });
        } catch (error) {
    ...
    // NEW: Set up real-time chat listener
                ...
                snapshot.docChanges().forEach((change) => {
                    if (change.type === 'added') {
                        ...
                        if (!document.querySelector(`[data-message-id="${change.doc.id}"]`)) {
                            addChatMessage(msg.username, msg.message, msg.time, false, change.doc.id, msg.replyTo);
                        }
                    }
                });
            });
    ...
    // UPDATED: Add chat message with reply and mention features
    function addChatMessage(username, message, time = null, isUserMessage = false, messageId = null, replyTo = null) {
        ...
        // NEW: Add reply indicator if this is a reply
        let replyIndicator = '';
        if (replyTo) {
            replyIndicator = `
        ...
        // NEW: Process mentions in message
        ...
        messageElement.innerHTML = `
            <div class="message-header">
                ...
                <div class="message-actions" style="margin-left: auto; display: flex; gap: 0.5rem;">
                    <button class="reply-btn" style="background: none; border: none; color: var(--accent); cursor: pointer; font-size: 0.8rem;" title="Reply">
                        ↪
                    </button>
                    <button class="whatsapp-btn" style="background: none; border: none; color: #25D366; cursor: pointer; font-size: 0.8rem;" title="Share via WhatsApp">
                        📱
                    </button>
                </div>
            </div>
        ...
        // NEW: Add event listeners for reply and WhatsApp buttons
        const replyBtn = messageElement.querySelector('.reply-btn');
        const whatsappBtn = messageElement.querySelector('.whatsapp-btn');
    ...
    // NEW: Process mentions in message text
    function processMentions(text) {
        // Convert @username to highlighted mentions
        return text.replace(/@(\w+)/g, '<span class="mention" style="color: var(--accent); font-weight: bold;">@$1</span>');
    }
    
    // NEW: Set reply message
    function setReplyMessage(message) {
        replyingToMessage = message;
        replyText.textContent = `Replying to ${message.username}: ${message.message}`;
        replyContainer.style.display = 'block';
        chatInput.focus();
        
        // Auto-add mention if not already mentioned
        if (!chatInput.value.includes(`@${message.username}`)) {
            chatInput.value = `@${message.username} ` + chatInput.value;
        }
    }
    
    // NEW: Share message on WhatsApp
    ...
    // UPDATED: Send message to global chat with reply support
    async function sendMessage() {
        ...
        if (message) {
            ...
            // NEW: Add reply data if replying to a message
