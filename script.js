...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function renderCourses(coursesToRender) {
        ...
        coursesToRender.forEach(course => {
            // FIX: Use string comparison for course IDs
            const isUnlocked = currentUser.unlockedCourses && 
                currentUser.unlockedCourses.some(unlockedId => 
                    unlockedId.toString() === course.id.toString()
                );
            const hasLiked = currentUser.likedCourses && 
                currentUser.likedCourses.some(likedId => 
                    likedId.toString() === course.id.toString()
                );
            const hasDisliked = currentUser.dislikedCourses && 
                currentUser.dislikedCourses.some(dislikedId => 
                    dislikedId.toString() === course.id.toString()
                );
            ...
            if (isUnlocked) {
                ...
                courseContent = `
                    ...
                    <div class="course-meta" style="display: flex; justify-content: space-between; align-items: center; margin-top: auto;">
                        ...
                        <button class="btn" data-course-id="${course.id}" data-course-title="${course.title}" style="background: var(--danger); color: white; padding: 0.7rem 1.5rem; border-radius: 12px; border: none; cursor: pointer; font-weight: bold; display: flex; align-items: center; gap: 0.6rem;">
    ...
    function openCourseModal(course) {
        ...
        confirmPurchase.dataset.coursePrice = course.price || 100;
    ...
    if (confirmPurchase) {
        confirmPurchase.addEventListener('click', async function() {
            ...
            const coursePrice = parseInt(this.dataset.coursePrice);
            ...
            if ((currentUser.lp || 0) >= coursePrice) {
                ...
                // FIX: Check if course is already unlocked
                const isAlreadyUnlocked = currentUser.unlockedCourses.some(id => 
                ...
                if (!isAlreadyUnlocked) {
                    ...
                    showSuccess('Course already unlocked!', false);
                }
            } else {
                purchaseError.style.display = 'block';
            }
    ...
    async function handleCourseInteraction(courseId, type) {
        try {
            ...
            if (type === 'like') {
                // FIX: Check if user already liked this course (using string comparison)
                const alreadyLiked = currentUser.likedCourses.some(id => 
                    id.toString() === courseId.toString()
                );
                ...
                if (alreadyLiked) {
                    ...
                    userUpdateData.likedCourses = currentUser.likedCourses.filter(id => 
                        id.toString() !== courseId.toString()
                    );
                    ...
                    const alreadyDisliked = currentUser.dislikedCourses.some(id => 
                        id.toString() === courseId.toString()
                    );
                    if (alreadyDisliked) {
                        updateData.dislikes = (courseData.dislikes || 1) - 1;
                        userUpdateData.dislikedCourses = currentUser.dislikedCourses.filter(id => 
                            id.toString() !== courseId.toString()
                        );
                ...
                // FIX: Check if user already disliked this course (using string comparison)
                const alreadyDisliked = currentUser.dislikedCourses.some(id => 
                    id.toString() === courseId.toString()
                );
            ...
            if (type === 'like') {
                ...
                if (alreadyLiked) {
                    currentUser.likedCourses = currentUser.likedCourses.filter(id => 
                        id.toString() !== courseId.toString()
                    );
                } else {
                    ...
                    if (alreadyDisliked) {
                        currentUser.dislikedCourses = currentUser.dislikedCourses.filter(id => 
                            id.toString() !== courseId.toString()
                        );
                    }
                }
    ...
    async function initializeChat() {
        ...
        // Add initial global chat messages in correct order
        globalChatMessages.forEach(msg => {
            addChatMessageToDOM(msg.username, msg.message, msg.time);
        });
        
        // Load global chat messages from Firestore in correct order
    ...
    // Load global chat from Firestore - FIXED: Load in ascending order
    async function loadGlobalChat() {
        try {
            // FIX: Load in ascending order for correct display
            const chatRef = db.collection('globalChat').orderBy('timestamp', 'asc');
            const snapshot = await chatRef.get();
            
            snapshot.forEach(doc => {
                const msg = doc.data();
                addChatMessageToDOM(msg.username, msg.message, msg.time, msg.replyToUsername, msg.replyToMessage);
            });
    ...
    // Set up real-time chat listener - FIXED: Handle new messages properly
    function setupChatListener() {
        // FIX: Listen for new messages in descending order but add them correctly
    ...
    // Add chat message to DOM - FIXED: Proper ordering
    function addChatMessageToDOM(username, message, time = null, replyToUsername = null, replyToMessage = null) {
        if (!chatMessagesContainer) return;
        ...
        // Check for mentions
        if (message && currentUser?.username && message.includes(`@${currentUser.username}`)) {
        ...
        // FIX: Always append to the end for correct ordering
        chatMessagesContainer.appendChild(messageElement);
        
        // Scroll to bottom only for new messages (not initial load)
        if (chatMessagesContainer.children.length > 10) {
            chatMessagesContainer.scrollTop = chatMessagesContainer.scrollHeight;
        }
    }
